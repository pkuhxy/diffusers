...
class FluxKontextPipeline(
    ...
    def __call__(
        ...
        # print("latents.shape",latents.shape)
        # print("image_latents.shape", image_latents.shape if image_latents is not None else None)
        # print("noised_latents.type",noised_latents[0].dtype)

        noise_images = []
        if if_output_noise:
            for noise_latent in noised_latents:
        ...
        if if_output_image_latents:
            ...
            latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
            image = self.vae.decode(latents, return_dict=False)[0]
            padded_tensor = self.image_processor.postprocess(image, output_type=output_type)
        ...
        if not return_dict:
            return (image,), noise_images
        
        if if_output_noise:
            return FluxPipelineOutput(images=image), noise_images
            
        if if_output_image_latents:
            return FluxPipelineOutput(images=image), padded_tensor
